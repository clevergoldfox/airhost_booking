var operation_data = JSON.parse(localStorage.getItem('operationData'))||[];
var sheetList = JSON.parse(localStorage.getItem('operationSheetList'))||[];
var sheetNum = sheetList.length;
var selSheet = 1;
...
async function draw_operation_chart(){
    ...
    if(operation_data.length == 0 || !operation_data[0].length){
        $("#opedate-chart").empty().append("<p class='text-center'>データがありません。</p>");
    } else {
        /* ================  bar chart ================*/
        var data = operation_data[selSheet-1]; // Skip the header row
        console.log(data);
...
function draw_reservation_calendar() {
    if (reservation_data.length == 0) {
        $("#reservation-calender-title").text("予約カレンダー");
        document.getElementById("calendar-thead").innerHTML = "<p class='text-center'>データがありません。</p>";
    } else {
    ...
    for (const key in disData) {
        ...
        if( i && i< length) {
            htmlstr += `<div class="col-xs-3 col-sm-3 col-md-3 col-lg-3 flex-column text-center">
                            ...
                            <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                                <p>予約数: ${disData[key]}件</p><p>増加率: ${Math.round((disData[key]-preValue)/(disData[key] || 1)*100)}%</p>
                            </div>
                        </div>`;
        } else {
            htmlstr += `<div class="col-xs-3 col-sm-3 col-md-3 col-lg-3 flex-column text-center">
...
function draw_ota_graph(){
    if(!otaChartData || Object.keys(otaChartData).length == 0){
        $("#ota-graph").empty().append("<p class='text-center'>データがありません。</p>");
        $("#ota-footer").empty();
    } else {
            ...
            Morris.Area({
                ...
                xLabelFormat: function (x) {
                    ...
                    if(type == "year"){
                        ...
                        let month = new Date(x).getMonth() +1 ; // Months are 0-indexed
                        // if (month > 12) {
                        //     month = month - 12;
                        ...
                        let month = new Date(x).getFullYear()-1900;
                        let disYear = year
                        // if(month > 12) {
                        //     month = month -12;
                        //     disYear = year/1 + 1;
        ...
        chartData.forEach((item, index) => {
            ...
            htmlstr += `<tr>
                            <td>${item.period}${type == "year"? "年":"月"}</td>
                            <td>${airbnbCount}</td>
                            <td>${airbnbRate}</td>
                            <td>${bookingCount}</td>
                            <td>${bookingRate}</td>
        ...
        $("#ota-footer").empty();
        let footerHtml = `<section class="col-xs-4 col-sm-4 col-md-4 col-lg-4">
									<label class="select">分析方式（年別、月別）
										<select name="otaAnalysisType" id="otaAnalysisType">
											<option value="year" selected>年別</option>
											<option value="month">月別</option>
										</select> <i></i> </label>
								</section>
								
								<section class="col-xs-3 col-sm-3 col-md-3 col-lg-3 d-none" id="otaSelYearDiv" >
									<label class="select">年選択 
										<select name="otaSelYear"	id="otaSelYear">
											<option value="2023" selected="true">2023年</option>
											<option value="2024">2024年</option>
											<option value="2025">2025年</option>
										</select> <i></i> </label>
								</section>
								<button class="btn btn-success" id="view-ota-chart" onclick="draw_ota_graph()">見る</button>`;
        $("#ota-footer").append(footerHtml);
    }
}
